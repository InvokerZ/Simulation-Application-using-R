...
while(jcount <= numpath) {
  ...
  BS_12_Val[jcount]<-max((max(ST_Vals[,jcount])-Strike),0)
  jcount=jcount+1
}
mean(BS_12_Val[1:numpath])*exp(-rf*TotalT)
alpha<-0.95
up<-mean(BS_12_Val[1:numpath])-sd(BS_12_Val[1:numpath])*MYNORM(1-(1-alpha)/2)/sqrt(numpath)
down<-mean(BS_12_Val[1:numpath])+sd(BS_12_Val[1:numpath])*MYNORM(1-(1-alpha)/2)/sqrt(numpath)
cat("CI is [",up,down,"]")
```
#payoff = {Max(S1,…,ST) – K}+ //path=10000
```{r}
S0<-100
rf<-0.03
Volatility<-0.25
...
while(jcount <= numpath) {
  ...
  ST_Rets[jcount]<-log(ST_Vals[12,jcount]/S0)
  BS_12_Val[jcount]<-max((ST_Vals[12,jcount]-min(ST_Vals[,jcount])),0)
  jcount=jcount+1
}
mean(BS_12_Val[1:numpath])*exp(-rf*TotalT)
alpha<-0.95
...
cat("CI is [",up,down,"]")
```
#payoff = { ST - Min(S1,…,ST)}+ //path=10000
```{r}
S0<-100
rf<-0.03
Volatility<-0.25
...
cat("CI is [",up,down,"]")
```
#repeat the four scenarios above by setting seed as -2000
#seed is -2000
```{r}
seed<--2000
numran<-1000000
